...
// ---------------------------------------------------------------------------
// Tier 2 broker panel (mxc-service via mxc-net.exe)
// ---------------------------------------------------------------------------
(() => {
  const out = document.getElementById('brokerOutput') as HTMLPreElement | null;
  const sidInput = document.getElementById('brokerAcSid') as HTMLInputElement | null;
  const btnVer = document.getElementById('btnBrokerVersion') as HTMLButtonElement | null;
  const btnAdd = document.getElementById('btnBrokerAddDemo') as HTMLButtonElement | null;
  const btnRem = document.getElementById('btnBrokerRemove') as HTMLButtonElement | null;
  if (!out || !btnVer || !btnAdd || !btnRem || !sidInput) return;

  const broker = (window as any).mxc;
  let lastPolicyId: string | null = null;

  const log = (label: string, res: any) => {
    const line = `[${new Date().toISOString().slice(11, 19)}] ${label}\n` +
      `  exit=${res?.code}  ok=${res?.success}\n` +
      (res?.stdout ? `  stdout: ${res.stdout.trim()}\n` : '') +
      (res?.stderr ? `  stderr: ${res.stderr.trim()}\n` : '');
    out.textContent = (out.textContent === '(no output yet)' ? '' : out.textContent) + line + '\n';
    out.scrollTop = out.scrollHeight;
  };

  btnVer.addEventListener('click', async () => {
    log('version', await broker.brokerVersion());
  });

  btnAdd.addEventListener('click', async () => {
    const sid = sidInput.value.trim();
    if (!sid.startsWith('S-1-15-2-')) {
      log('add', { code: -1, success: false, stderr: 'Enter a valid AppContainer SID first (S-1-15-2-...)' });
      return;
    }
    const rules = [
      'allow:tcp:140.82.112.0:20:443',
      'allow:tcp:1.1.1.1:32:443',
    ];
    const res = await broker.brokerAddPolicy(sid, 'block', rules, 0);
    log('add', res);
    if (res?.success && res.stdout) {
      try {
        const parsed = JSON.parse(res.stdout);
        if (parsed.policy_id) {
          lastPolicyId = parsed.policy_id;
  ...
  btnRem.addEventListener('click', async () => {
    ...
    const res = await broker.brokerRemovePolicy(lastPolicyId);
    log(`remove ${lastPolicyId}`, res);
    lastPolicyId = null;
