...
// ---------------------------------------------------------------------------
// Tier 2 broker (mxc-service) IPC
//
// The playground shells out to `mxc-net.exe` rather than reimplementing the
// CBOR-over-named-pipe protocol in TypeScript. mxc-net ships alongside the
// other sandbox binaries (see package.json extraResources) and is wired up
// to the same elevated service that the MSI installs system-wide.
// ---------------------------------------------------------------------------
function resolveMxcNetPath(): string {
  const fs = require('fs');
  const arch = process.arch === 'arm64' ? 'arm64' : 'x64';
  const candidates: string[] = [];
  if ((process as any).resourcesPath) {
    candidates.push(path.join((process as any).resourcesPath, 'bin', arch, 'mxc-net.exe'));
  }
  const repoRoot = path.join(__dirname, '..', '..', '..');
  const triple = process.arch === 'arm64' ? 'aarch64-pc-windows-msvc' : 'x86_64-pc-windows-msvc';
  candidates.push(
    path.join(repoRoot, 'src', 'target', triple, 'release', 'mxc-net.exe'),
    path.join(repoRoot, 'src', 'target', 'release', 'mxc-net.exe'),
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error(`mxc-net.exe not found. Looked in:\n  ${candidates.join('\n  ')}`);
}

function runMxcNet(args: string[]): Promise<{ success: boolean; stdout: string; stderr: string; code: number }> {
  const { execFile } = require('child_process');
  return new Promise((resolve) => {
    let exe: string;
    try {
      exe = resolveMxcNetPath();
    } catch (e: any) {
      resolve({ success: false, stdout: '', stderr: e.message, code: -1 });
      return;
    }
    execFile(exe, args, { timeout: 10_000 }, (err: any, stdout: string, stderr: string) => {
      resolve({
        success: err == null,
...
ipcMain.handle('broker-version', async () => runMxcNet(['--json', 'version']));
...
ipcMain.handle(
  ...
  async (
    ...
    const args = ['--json', 'add', acSid, '--default', defaultPolicy, '--sandbox-pid', String(sandboxPid)];
    for (const r of rules) {
      args.push('--rule', r);
    }
    return runMxcNet(args);
  },
);

ipcMain.handle('broker-remove-policy', async (_event, policyId: string) =>
  runMxcNet(['--json', 'remove', policyId]),
);
